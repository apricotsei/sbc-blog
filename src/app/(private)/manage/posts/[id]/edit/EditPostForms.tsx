...
export default function EditPostForm({post} : EditPostFormProps) {
    ...
    //コンポーネントが破棄されるかimagePreview変更時にプレビューURLを解法
    useEffect(() => {
        return () => {
    ...
    return (
        <div className="container mx-auto mt-10">
            ...
            <form action={formAction} encType="multipart/form-data" className="space-y-4" onSubmit={() => console.log('フォームがsubmitされた')}>
                <div>
                ...
                <div>
                    ...
                    <Input 
                        ...
                        accept="image/*"
                        name="topImage"
                        onChange={handleImageChange}  //プレビューの画像が切り替わる
                    />
                    {imagePreview && (
                        <div className="mt-2">
                            <Image
                ...
                <div>   
                    <Button type="button" onClick={() => setPreview(!preview)}>
                        {preview ? 'プレビューを閉じる' : 'プレビューを表示'}     {/* 参考演算子(trueかfalseか) */}
                    </Button> 
                </div> 
                {preview && (
                    <div className="border p-4 bg-grey-50 prose max-w-none">
                        <ReactMarkdown
